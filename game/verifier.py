...
def main(argv):
  ...
  for _ in range(T):
    N = vr.ReadInt(1, 10**9)
    vr.ReadSpace()
    M = vr.ReadInt(1, 2 * 10**5)
    vr.ReadSpace()
    K = vr.ReadInt(1, 2 * 10**5)
    vr.ReadEndl()
    ...
    assert len(pairset) == M, ('Pairs (R, C) are not distinct')


  assert sum_m <= 2 * 10**5, ("sum of M across all test cases exceeds 200'000")
