...
int main()
{
	...
	for(int qr = 0; qr < (int)queries.size(); qr++)
	{
		...
		long long mn = segtree.qry(p * 2 + 1, N * 2) - (long long)Ts * p + Tf - (long long)Tw * 2 * fenwickQry(1, p - 1);
		if(pref[N - 1] - (long long)D * N >= w)
		{
			mn = min(mn, (long long)Ts * (N - p) + (long long)Tw * 2 * fenwickQry(p, N - 1));
		}
		if(pref[N] - (long long)D * N >= w)
		{
			mn = min(mn, (long long)Ts * (N - p) + (long long)Tw * 2 * (fenwickQry(p, N - 1) + 1));
		}
		...
		segtree.ins(p * 2 - 1 + e, mn + (long long)Ts * (p - 1) + (long long)Tw * 2 * fenwickQry(1, p - 1));
	}
}
